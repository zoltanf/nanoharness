...
def _readme_body(*, source_repo: str, tap_repo: str, cask_token: str, formula_name: str) -> str:
    return "\n".join(
        [
            ...
            "```",
            "",
            "The cask installs both `NanoHarness.app` and the `nanoh` terminal command.",
            "",
            ...
            "NanoHarness requires Ollama to be installed separately.",
            "",
            "If macOS blocks `NanoHarness.app` on first launch because the build is not notarized yet, remove the quarantine flag:",
            "",
            "```bash",
            'sudo xattr -r -d com.apple.quarantine "/Applications/NanoHarness.app"',
            "```",
            "",
            "Or open `System Settings` -> `Privacy & Security` and choose `Open Anyway` for `NanoHarness.app`.",
            "",
